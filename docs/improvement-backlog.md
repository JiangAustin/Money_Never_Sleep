...
| MNS-BL-020 | 待设计 | P1 | 真实行情回测数据源 | 阶段 7.1 只接受调用者传入价格序列，不接真实 K 线 provider | 回测可以直接基于真实历史行情运行，减少手工输入 | 先完成真实 K 线 provider，再把它接入 backtest request | 阶段 7.1 后续、`domains/market_data/` |
