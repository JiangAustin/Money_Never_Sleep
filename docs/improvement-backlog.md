...
| MNS-BL-058 | 已完成 | P0 | 真实 TradingAgents 主路径强化 | 真实 runner 已能调用 graph，但默认角色、完整报告映射和运行 diagnostics 还不足以支撑上线验收 | 让真实 TradingAgents 成为 `auto` 的清晰主路径，并让用户看见命中、模型、角色、耗时和回退原因 | 已完成 7 analyst 默认配置、TradingAgents-astock 默认配置合并、完整 state 到 `AgentView` 的映射、上下文快照 diagnostics 和 Web 命中摘要；验证：runner/API/Web 定向测试与 JS check 通过 | `services/api/money_api/integrations/tradingagents_runner.py`、`apps/web/src/app.js` |
| MNS-BL-059 | 待设计 | P1 | Money_Never_sleep provider 注入 TradingAgents 工具层 | 当前真实 TradingAgents 先使用其自身 A 股工具链，Money_Never_sleep 的 `DataContext` 只作为快照保存 | 让真实多 Agent 流程能直接消费本仓库 provider、事件流、可信度和风险约束 | 先完成真实 smoke 和上线主路径，再评估通过 adapter/provider 注入而不是直接改外部项目 | `TradingAgents-astock`、`ResearchToolService` |
