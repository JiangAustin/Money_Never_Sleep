...
- CLS 市场快讯
- 新浪公告标题
- `auto` 深度引擎（优先 TradingAgents，失败回退 mock）

如果下一个 agent 只想快速判断“现在系统最像什么”，答案是：

默认链路已经具备“真实行情 + 真实资讯/公告输入 + 真实深度引擎优先尝试”的产品雏形，但公司事件仍停留在标题/摘要层，尚未进入结构化事件流。

## 建议接手顺序

如果继续沿最近的路线推进，推荐按下面顺序做，而不是再回到宽泛探索：

1. 先补结构化事件流：业绩预告、担保、减持、解禁这类高价值公司事件。
2. 再补 `auto` 模式前端可见性：明确显示本次分析是否命中真实 TradingAgents、为何回退。
3. 最后再考虑更重的扩展：资金流、龙虎榜、公告正文原文抓取、结构化事件分类。
...
如果你的目标是继续默认 runtime 能力，而不是做全新模块，优先直接跳到本文档中的“最近连续推进摘要”和阶段 5.16-5.19、6.1-6.2 部分，不必先从阶段 0 重新读到尾。
