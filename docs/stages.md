...
阶段 5.9 已完成。当前系统已具备带超时回收的异步 HTTP 分析闭环：
...
4. 新增 `timeout_s` 和 `started_at`，任务在读取或轮询时超过超时窗口会自动转为 `failed`。
5. Web/桌面在连接 HTTP API 时，前端优先走任务模式，而不是直接等待同步 `POST /analysis` 长阻塞。
6. 取消不是底层线程/外部引擎的强制中断；自动重试和真正的后台 watchdog 仍不属于当前切片。
...
建议下一步在两个方向中二选一：补真实 TradingAgents smoke 与自动重试/真正的后台 watchdog，或继续回测与数据层的真实化增强。
