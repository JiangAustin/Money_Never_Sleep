...
阶段 5.10 已完成。当前系统已具备带后台回收和自动重试的异步 HTTP 分析闭环：
...
4. 新增 `timeout_s`、`started_at`、`retry_count` 和 `max_retries`，任务可在后台 watchdog 扫描时自动超时失败。
5. 当任务由于超时或执行异常失败且仍有剩余次数时，服务端会自动派生 retry task。
6. 取消不是底层线程/外部引擎的强制中断；仍未实现复杂退避策略、分布式 worker 或真正的跨进程 watchdog。
...
建议下一步在两个方向中二选一：补真实 TradingAgents smoke 与更细粒度的重试/退避策略，或继续回测与数据层的真实化增强。
