...
第一版使用离线 mock 数据模拟后端 `AnalysisReport` 契约，支持本地发起分析、查看最近报告和报告详情。启动 HTTP API 后，可用 `index.html?api=http://127.0.0.1:8000` 切换到真实后端调用；当前前端会优先创建异步分析任务并轮询状态，请求失败时会回退到离线 mock。任务记录默认会持久化到本地缓存目录，服务重启后仍可被后端查询；页面还提供最小的取消当前任务和重试失败任务入口。
...
- 将任务控制继续扩展为更丰富的历史任务面板、重试策略配置和批量管理。
